...
# In[2]:
print("Currently running: LSTM_Model+SingleSubject_LoopAll_hpc.py")
print("\n")
...
#     data_matrix_gon = np.dstack([matrix.values for matrix in data_matrices_gon])
    ...
    label_matrix = pd.read_csv(f"{path_labels_ik}/{number}_ik_hip_flexion_l_y_labels.csv")
...
def save_cm(cm, subject, r, base_save_path):
    ...
    save_path_csv = os.path.join(base_save_path,
                                 f"{subject}/cm_csv/",
                                 f'{subject}_50ms_LSTM_IK+IMU_Hip_WithAngvel_cm_{r+1}.csv')
    # check if the path exists
...
def display_percentage_confusion_matrix(cm, subject, accuracy, r, base_save_path):
    ...
    with plt.style.context('classic'):
        ...
        # plt.title(f'Subject: {subject} Accuracy: {100*accuracy:.2f}\nModel: LSTM\nIK_Joints: Hip+Knee+Ankle(Bilateral) IMU: Thigh(Bilateral)', color='black', size=16)
        plt.title(
...
def run_experiment(repeats=20):
    ...
    for subject in subjects:
        # empty results dict for each subject
        results = {}
        ...
        for r in range(repeats):
            ...
            model, train_loss, accuracy = evaluate_model(trainX, trainy, testX, testy)
            train_accuracy = model.evaluate(trainX, trainy, verbose=0)[1]
            pred = np.argmax(model.predict(testX), axis=-1)
            true = np.argmax(testy, axis=1)
            cm = confusion_matrix(true, pred)
            
            # Append current round results to DataFrame
            round_result = pd.DataFrame({'Round': [r+1], 'Training_Accuracy': [train_accuracy], 'Test_Accuracy': [accuracy]})
        ...
        save_path_csv = os.path.join(base_save_path,
                                     ...
                                     f'{subject}_50ms_LSTM_IK+IMU_Hip_WithAngvel_results.csv')
        os.makedirs(os.path.dirname(save_path_csv), exist_ok=True)
